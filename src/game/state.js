...
export function migrateLegacyRun(legacy, profile) {
  ...
  Object.assign(run, {
    ...
    firstClearAchieved: Boolean(legacy.firstClearAchieved),
    firstClearElapsed: Number.isFinite(legacy.firstClearElapsed) ? legacy.firstClearElapsed : null,
    hazardRoundPicks: Array.isArray(legacy.hazardRoundPicks) ? legacy.hazardRoundPicks.filter(Number.isInteger) : [],
